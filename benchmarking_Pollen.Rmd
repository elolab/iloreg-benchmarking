...
data <- log(raw_data+1) # add pseudocount and take logarithm
data <- Matrix::Matrix(as.matrix(data),sparse = TRUE)
data <- data[!(grepl("ERCC-",rownames(data))|grepl("Spike",rownames(data))),] # Remove spike-ins
raw_data <- raw_data[!(grepl("ERCC-",rownames(raw_data))|grepl("Spike",rownames(raw_data))),] # Remove spike-ins

ground_truth <- factor(unlist(lapply(strsplit(colnames(data),"_"),function(x) x[2])))
```
...
```{r fig_Pollen_ILoReg, fig.height = 10, fig.width = 12, fig.align = "center"}
...
```{r fig_Pollen_Seurat, fig.height = 3, fig.width = 8, fig.align = "center"}
...
```{r fig_Pollen_SC3, fig.height = 3, fig.width = 8, fig.align = "center"}
...
```{r fig_Pollen_CIDR, fig.height = 3, fig.width = 8, fig.align = "center"}
...
```{r fig_Pollen_RaceID3, fig.height = 3, fig.width = 8, fig.align = "center"}
...
```{r}
save(clustering_iloreg,tsne_iloreg,file="Pollen_ILoReg_clustering_dimensionality_reduction.RData")
save(clustering_Seurat,tsne_Seurat,file="Pollen_Seurat_clustering_dimensionality_reduction.RData")
save(clustering_SC3,tsne_SC3,file="Pollen_SC3_clustering_dimensionality_reduction.RData")
save(clustering_cidr,pca_cidr,file="Pollen_CIDR_clustering_dimensionality_reduction.RData")
save(clustering_raceid3,tsne_raceid3,file="Pollen_RaceID3_clustering_dimensionality_reduction.RData")
